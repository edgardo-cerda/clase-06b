...
::: {.column width="50%"}
![](imagenes/visualizacion_atencion.png){width="350" height="500"}
:::
:::
...
::: medium-par
La atención se implementa con tres matrices aprendibles.
:::

::: {.incremental .medium-par}
Para cada token se definen 3 vectores:

-   **Query (Q)**: *"¿Qué información necesito para entender el
    token?"* - lo que estoy buscando.
-   **Key (K)**: *"¿Qué información tiene el token que puede ayudar a
    entender otros tokens?"* - lo que cada token ofrece.
-   **Value (V)**: *"¿Qué información entrego si lo eligen eligen?"* -
    el contenido que se pasa.
:::
...
![](imagenes/esquema_QKV.png){width="800" height="300"}

## Queries, Keys y Values

$$\text{Attention}(Q, K, V) = \text{softmax}\!\left(\frac{QK^\top}{\sqrt{d_k}}\right) V$$

![](imagenes/esquema_QKV2.png){width="850" height="300"}
...
::: {.incremental .medium-par}
El mecanismo de atención corresponde a una "cabeza de atención"
:::
...
![](imagenes/arquitectura_simplificada.png){width="800" height="400   "}
...
::: fragment
Pero cada mitad resultó ser más útil por separado, y para propósitos
diferentes.
:::
:::
...
-   No genera predicciones de texto: su salida son vectores, no palabras
...
::: {.incremental .medium-par}
-   HuggingFace `transformers` es una biblioteca de código abierto que
    proporciona herramientas para crear, entrenar y desplegar modelos de
    inteligencia artificial de última generación.

-   Permite hacer fine-tuning de BETO en pocas líneas. El pipeline
    completo tiene cinco etapas:
:::
...
![](imagenes/entrenamiento_modelo.png){width="700" height="400"}

## Resultados

![](imagenes/resultado_modelo.png){width="450" height="500"}
...
4.  **Encoder vs. Decoder**: para clasificación, usamos encoders
    (BERT/BETO).
...
7.  **Aplicaciones**: codificación automática de ocupaciones (CIUO-08).
:::
