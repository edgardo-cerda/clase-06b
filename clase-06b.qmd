...
3. **Arquitectura Transformer:** Bloque transformer, embeddings posicionales
...
<!-- El paper original ("Attention Is All You Need", Vaswani et al., 2017) propuso una arquitectura encoder-decoder para traducción automática. Hoy usamos variantes de ella, pero la estructura base es similar. -->
...
::: {.fragment}
El Transformer original (2017) se diseñó para traducción automática, por lo que tenía dos mitades: encoder y decoder. Encoder "codificaba" la información de un idioma en un vector numérico, y el decoder tomaba esto y lo "decodificaba" en otro idioma.
:::

::: {.fragment}
Con el tiempo se descubrió que cada mitad es útil por separado.
:::


## Modelos actuales (Claude, GPT-4, etc.)

Claude, GPT-4, Gemini y todos los grandes modelos de lenguaje actuales son, en su núcleo, transformers decoder-only entrenados con predicción de siguiente token sobre cantidades enormes de texto, seguido de refinamiento con retroalimentación humana (RLHF o variantes).

La arquitectura del paper de 2017 es reconocible, pero hay décadas de mejoras acumuladas: normalización de capas, embeddings posicionales aprendidos, atención con ventanas, mezcla de expertos (MoE), contextos de millones de tokens, y mucho más.
