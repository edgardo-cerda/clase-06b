...
```
1. `texto` es la concatenación de ocupación y tareas. Usar ambos campos mejora la clasificación porque el modelo dispone de más contexto que solo el nombre del cargo.
...
```
1. PyTorch necesita enteros contiguos desde 0. `LabelEncoder` hace ese mapeo automáticamente: si en los datos solo aparecen los grupos 1, 2, 3 y 5, los convierte a 0, 1, 2, 3. El objeto `le` guarda el mapeo inverso para recuperar los códigos CIUO originales al final.
2. `stratify` garantiza que la proporción de cada grupo CIUO sea la misma en train y test, lo que es importante cuando hay clases con pocos ejemplos.
...
```
1. 64 tokens es suficiente para glosas cortas de ocupación y ahorra memoria en GPU.
2. Sin `set_format`, el `Trainer` puede fallar al intentar armar los batches: los tensores de PyTorch deben ser explícitamente del tipo correcto.

## Paso 4: modelo, métricas y entrenamiento
...
trainer.train()
```
1. Al cargar el modelo verán avisos sobre pesos no utilizados (los de MLM) y pesos faltantes (la cabeza de clasificación). Ambos son normales.
2. F1 macro pondera cada clase por igual, independiente de su tamaño. Es más informativo que la accuracy cuando las clases están desbalanceadas.
3. Learning rate entre 2e-5 y 5e-5 es estándar para fine-tuning de BERT. Valores más altos pueden degradar los pesos pre-entrenados.
4. Evita que el Trainer intente conectarse a Weights & Biases. Si no se incluye, puede generar errores en entornos sin esa librería configurada.
...
print(reporte)
```
1. `le.classes_` contiene los códigos CIUO originales en el orden en que `LabelEncoder` los asignó. Así cada fila del reporte muestra el gran grupo CIUO real, no un índice interno.
...
```
1. `pipeline` devuelve `"LABEL_0"`, `"LABEL_1"`, etc. — índices internos, no códigos CIUO. `le.classes_[idx]` recupera el código CIUO original a partir de ese índice.

## Resultados esperados

::: {.medium-par}
Con el dataset de auditorías de Nomenclaturas (~50.000 glosas), resultados orientativos:
:::

::: {.medium-par}
| Nivel CIUO | Clases | Accuracy | F1 macro |
|---|---|---|---|
| **1 dígito** | 10 | 85–92% | 80–88% |
| **2 dígitos** | ~43 | 75–85% | 68–78% |
| **4 dígitos** | ~436 | 60–75% | 45–65% |
:::

::: {.fragment .nota-box}
El F1 macro es el indicador más relevante: pondera cada gran grupo por igual. La accuracy puede ser alta aunque el modelo falle en grupos minoritarios.
:::

::: {.fragment .callout-note .small-callout}
Estos números son referenciales, basados en la literatura y experiencia con datos similares. Los resultados reales dependen de la calidad y distribución del dataset de entrenamiento.
:::
...
7. **Aplicaciones en CPV2024**: codificación automática de ocupaciones (CIUO-08) y rama de actividad (CAENES).
:::
